...
# ------------------- پارامترهای مقاله (جداول I-III) -------------------
# جدول II: پارامترهای کلیدها
a = 0
delta = 1e-4
mu = 0.01
lambda_ = 5
...
# جدول III: پارامترهای رله
P_p0 = 0.0069  # تک حفاظت

# جدول I: ترکیب کلیدها
n_local_cbs = 3  # ۱ اصلی + ۲ پشتیبان
n_remote_cbs = 4  # ۱ اصلی + ۳ پشتیبان
P_lk = P_rk = 0.81
P_lk_breaker_half = P_lk * 0.5
P_rk_breaker_half = P_rk * 0.5


# ------------------- مدل‌سازی شبکه ۶۸-باس -------------------
def create_68bus_network():
    ...
    # خطوط ارتباطی (فرضی)
    lines = [
        {'from_bus': 26, 'to_bus': 53, 'length_km': 100, 'r_ohm_per_km': 0.01, 'x_ohm_per_km': 0.1, 'c_nf_per_km': 0.1,
         'max_i_ka': 1, 'name': 'Tie-Line_27-54', 'relay': 'overcurrent'},
        {'from_bus': 15, 'to_bus': 45, 'length_km': 100, 'r_ohm_per_km': 0.01, 'x_ohm_per_km': 0.1, 'c_nf_per_km': 0.1,
         'max_i_ka': 1, 'name': 'Tie-Line_16-46', 'relay': 'overcurrent'},
        {'from_bus': 35, 'to_bus': 50, 'length_km': 100, 'r_ohm_per_km': 0.01, 'x_ohm_per_km': 0.1, 'c_nf_per_km': 0.1,
         'max_i_ka': 1, 'name': 'Tie-Line_36-51', 'relay': 'directional'},
        {'from_bus': 25, 'to_bus': 55, 'length_km': 100, 'r_ohm_per_km': 0.01, 'x_ohm_per_km': 0.1, 'c_nf_per_km': 0.1,
         'max_i_ka': 1, 'name': 'Tie-Line_26-56', 'relay': 'directional'},
        {'from_bus': 5, 'to_bus': 60, 'length_km': 100, 'r_ohm_per_km': 0.01, 'x_ohm_per_km': 0.1, 'c_nf_per_km': 0.1,
         'max_i_ka': 1, 'name': 'Tie-Line_6-61', 'relay': 'directional'}
    ]
    for line in lines:
...
def calculate_risk(net, line_id, P_p0, P_lk, P_rk, config='Single Bus', fault='Fault'):
    ...
    L2 = P_Gi * LS_G
    results.append(
        {'Combination': '2 (Local CB Failure)', 'Probability': P_Gi, 'Load Loss (MW)': LS_G, 'Risk (MW)': L2})

    P_Gj = (1 - P_p0) * P_local_success * P_remote_failure
    L3 = P_Gj * LS_G
    results.append(
        {'Combination': '3 (Remote CB Failure)', 'Probability': P_Gj, 'Load Loss (MW)': LS_G, 'Risk (MW)': L3})
...
def simulate_table_ix(net_single, net_breaker_half):
    ...
    for idx, line in net_single.line.iterrows():
        ...
        # breaker-and-a-half
        _, L_total_breaker = calculate_risk(net_breaker_half, idx, P_p0, P_lk_breaker_half, P_rk_breaker_half,
                                            config='Breaker-and-a-Half', fault=line_name)
        results.append({
            'Line': line_name,
...
def simulate_table_xi(net_breaker_half):
    ...
    for idx, line in net_breaker_half.line.iterrows():
        line_name = line['name']
        results_breaker, _ = calculate_risk(net_breaker_half, idx, P_p0, P_lk_breaker_half, P_rk_breaker_half,
                                            config='Breaker-and-a-Half', fault=line_name)
...
print("\nجدول XI (ریسک breaker-and-a-half):")
